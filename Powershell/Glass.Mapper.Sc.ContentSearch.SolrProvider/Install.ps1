...
if($scVersion){	
	...
	$gmsPath = "{0}\lib\{1}\{2}" -f $installPath, $scVersion, "Glass.Mapper.Sc.ContentSearch.SolrProvider.dll";
	
	RemovingExisting "Glass.Mapper.Sc.ContentSearch.SolrProvider";
	AddReference $gmsPath "Glass.Mapper.Sc.ContentSearch.SolrProvider";

}
else{
	Write-Host "ERROR: Could not locate Sitecore.Kernel.dll, please add reference before installing Glass.Mapper.Sc.ContentSearch.SolrProvider";
}
