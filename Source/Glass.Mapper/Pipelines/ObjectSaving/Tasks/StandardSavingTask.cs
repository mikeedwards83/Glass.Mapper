...
namespace Glass.Mapper.Pipelines.ObjectSaving.Tasks
{
    ...
    public class StandardSavingTask : IObjectSavingTask
    {
        ...
        public void Execute(ObjectSavingArgs args)
        {
            ...
            AbstractDataMappingContext dataMappingContext = GetDataMappingContext(args, savingContext);


            savingContext.Config.Properties.ForEach(x => x.Mapper.MapPropertyToCms(dataMappingContext));
        }

        internal Func<ObjectSavingArgs, AbstractTypeSavingContext, AbstractDataMappingContext> GetDataMappingContext =
            (args, savingContext) => args.Service.ObjectFactory.CreateDataMappingContext(savingContext);
    }
}
