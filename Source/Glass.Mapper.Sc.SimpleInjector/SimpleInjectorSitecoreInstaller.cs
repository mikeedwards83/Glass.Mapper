...
namespace Glass.Mapper.Sc.SimpleInjector
{
    ...
    /// </summary>
    public class SimpleInjectorMapSitecoreInstaller : SitecoreInstaller
    {
        public SimpleInjectorMapSitecoreInstaller(Config config, Container container)
            : this(config, new DependencyResolver(container))
        {
        }

        protected SimpleInjectorMapSitecoreInstaller(Config config, IDependencyHandler dependencyRegistrar)
