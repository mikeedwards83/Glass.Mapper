...
namespace Glass.Mapper.Sc.Maps
{
    ...
    /// The type to map
    /// </typeparam>
    public abstract class SitecoreGlassMap<T> : AbstractGlassMap<SitecoreType<T>> where T : class
    {
        ...
        {
            SitecoreGlassMap<TK> sitecoreMap = DependencyResolver.Resolve<SitecoreGlassMap<TK>>();
            if (sitecoreMap == null)
            {
                // todo: unit test this
